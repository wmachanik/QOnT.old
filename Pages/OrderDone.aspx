<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
  CodeBehind="OrderDone.aspx.cs" Inherits="QOnT.Pages.OrderDone" %>
...
<asp:Content ID="cntOrderDoneBdy" ContentPlaceHolderID="MainContent" runat="server">
  ...
  <asp:UpdatePanel ID="updtpnlOrderDone" runat="server" ChildrenAsTriggers="true">
    <ContentTemplate>
      <asp:Panel ID="pnlOrderDetails" runat="server">
              ...
              <table>
                <tbody>
                  <tr>
                    ...
                    <td class="TblLHCol-first">DeliveryDate:
                    </td>
                ...
                <asp:TemplateField HeaderText="Item" SortExpression="ItemID">
                  <EditItemTemplate>
                    <asp:DropDownList ID="ddlItemDesc" runat="server" DataSourceID="sdsItemTypes" DataTextField="ItemDesc"
                      DataValueField="ItemTypeID" AppendDataBoundItems="True" SelectedValue='<%# Bind("ItemID") == null ? 0 : Bind("ItemID") %>'>
                ...
                <asp:TemplateField HeaderText="Packaging" SortExpression="PackagingID">
                  <EditItemTemplate>
                    <asp:DropDownList ID="ddlPackaging" runat="server" AppendDataBoundItems="true" DataSourceID="sdsPackagingTypes"
                      DataTextField="Description" DataValueField="PackagingID" SelectedValue='<%# Bind("PackagingID") == null ? 0 : Bind("PackagingID")  %>'>
                      <asp:ListItem Value="0">n/a</asp:ListItem>
                    </asp:DropDownList>
                  </EditItemTemplate>
                  <ItemTemplate>
        ...
        <span class="simpleForm" style="float: right; text-align: center; padding: 16px; padding-left: 40px; padding-right: 40px">
          <asp:Button ID="btnDone" Text="Done" runat="server" OnClick="btnDone_Click" /><br
        ...
        <table class="TblWhite">
          <tbody>
            <tr>
              <td>Stock:</td>
              <td><asp:TextBox ID="tbxStock" runat="server" Width="5em" /></td>
            </tr>
            <tr>
              <td>Cup Count:</td>
              <td><asp:TextBox ID="tbxCount" runat="server" Width="5em" /></td>
            </tr>
          </tbody>
        </table>
        <%--    </div>--%>
      ...
      <asp:Panel ID="pnlCustomerDetailsUpdated" runat="server" Visible="false">
        <table class="TblNoBorder">
          <thead>
            <tr>
              ...
              <td><asp:Label ID="tbxCustomerName" Text="" runat="server" /></td>
            </tr>
          </thead>
        </table>
        ...
        <br />
        <asp:Button ID="btnReturnToDeliveres" Text="Return to Delivery Sheet" 
          runat="server" onclick="btnReturnToDeliveres_Click" />
      </asp:Panel>
    </ContentTemplate>
    </asp:UpdatePanel>
    <asp:SqlDataSource ID="sdsOrderDoneHeader" runat="server" ConnectionString="<%$ ConnectionStrings:Tracker08ConnectionString %>"
    ...
    <asp:SqlDataSource ID="sdsOrderDoneLines" runat="server" ConnectionString="<%$ ConnectionStrings:Tracker08ConnectionString %>"
      DeleteCommand="DELETE FROM [TempOrdersLinesTbl] WHERE [TOLineID] = ?" InsertCommand="INSERT INTO [TempOrdersLinesTbl] ([ItemID], [Qty], [PackagingID], [TOLineID]) VALUES (?, ?, ?, ?)"
      ProviderName="<%$ ConnectionStrings:Tracker08ConnectionString.ProviderName %>"
    ...
    <asp:SqlDataSource ID="sdsItemTypes" runat="server" ConnectionString="<%$ ConnectionStrings:Tracker08ConnectionString %>"
      ProviderName="<%$ ConnectionStrings:Tracker08ConnectionString.ProviderName %>"
      SelectCommand="SELECT [ItemTypeID], [ItemDesc] FROM [ItemTypeTbl] ORDER BY [ItemEnabled], [SortOrder], [ItemDesc]">
    </asp:SqlDataSource>
    <asp:SqlDataSource ID="sdsPackagingTypes" runat="server" ConnectionString="<%$ ConnectionStrings:Tracker08ConnectionString %>"
      ProviderName="<%$ ConnectionStrings:Tracker08ConnectionString.ProviderName %>"
